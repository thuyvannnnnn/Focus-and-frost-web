...
<body class="h-full relative overflow-hidden font-sans bg-gray-50">
  ...
  >
    ...
    <button
      aria-label="Mở lịch sử bánh đã trang trí"
    ...
    >
      ...
      />
      <img
        alt="Bánh mousse ba lớp, hình tròn, màu kem trắng và hồng nhạt, trang trí đơn giản"
      ...
      />
      <div
        aria-label="Khu vực trang trí đồ trang trí trên bánh"
    ...
    >
      ...
      >
        <h2 class="text-xl font-bold mb-4 text-center" id="history-title">
          Lịch sử bánh đã trang trí
        </h2>
        <div
          class="flex flex-col gap-4 overflow-y-auto no-scrollbar"
          id="history-list"
        >
          <p class="text-center text-gray-600">Chưa có bánh đã trang trí nào trong lịch sử.</p>
        </div>
        <button
    ...
    <button
      id="trash-button"
      aria-label="Xóa đồ trang trí"
      title="Kéo đồ trang trí vào đây để xóa"
      type="button"
    >
      <i class="fas fa-trash-alt text-2xl"></i>
    </button>
    <div id="user-info" role="region" aria-live="polite" aria-atomic="true" aria-label="Thông tin người dùng Google">
      <img src="" alt="Ảnh đại diện người dùng Google" id="user-avatar" />
  ...
  <script>
    (() => {
      ...
      const defaultPlateInventory = [
        {
          ...
          alt: "Đĩa màu be nhạt, hình tròn, kiểu đơn giản"
        }
      ];
      const defaultDecorateInventory = [
        {
          ...
          alt: "Kem trang trí, hình ảnh trang trí nhỏ màu kem trắng"
        }
      ];

      // Load inventories from sessionStorage or initialize with default if not present
      let cakeInventory = [];
      let plateInventory = [];
      ...
      function initializeDataIfNeeded() {
        if (!sessionStorage.getItem("cakeInventory")) {
          sessionStorage.setItem("cakeInventory", JSON.stringify(defaultCakeInventory));
        }
        if (!sessionStorage.getItem("plateInventory")) {
          sessionStorage.setItem("plateInventory", JSON.stringify(defaultPlateInventory));
        }
        if (!sessionStorage.getItem("decorateInventory")) {
          sessionStorage.setItem("decorateInventory", JSON.stringify(defaultDecorateInventory));
        }
        if (!sessionStorage.getItem("rewardInventory")) {
          sessionStorage.setItem("rewardInventory", JSON.stringify([
        ...
        if (!sessionStorage.getItem("decoratedCakes")) {
          sessionStorage.setItem("decoratedCakes", JSON.stringify([]));
        }
      }

      function loadInventories() {
        try {
          const cakeRaw = sessionStorage.getItem("cakeInventory");
          cakeInventory = cakeRaw ? JSON.parse(cakeRaw) : [];
          const plateRaw = sessionStorage.getItem("plateInventory");
          plateInventory = plateRaw ? JSON.parse(plateRaw) : [];
          const decorateRaw = sessionStorage.getItem("decorateInventory");
        ...
        try {
          const historyData = sessionStorage.getItem("decoratedCakes");
      ...
      function autoSave() {
        ...
        sessionStorage.setItem("rewardInventory", JSON.stringify(combinedRewardInventory));
        sessionStorage.setItem("decoratedCakes", JSON.stringify(history));
        sessionStorage.setItem("plateInventory", JSON.stringify(plateInventory));
        sessionStorage.setItem("cakeInventory", JSON.stringify(cakeInventory));
        sessionStorage.setItem("decorateInventory", JSON.stringify(decorateInventory));
      }
      ...
      function renderSelectCake() {
        ...
        if (cakeInventory.length === 0) {
          ...
          noCakeMsg.textContent = "Không còn bánh nào để chọn.";
      ...
      function renderSelectDecorations() {
        ...
        const title = document.createElement("h2");
        title.textContent = "Chọn đồ trang trí";
        title.className =
          "text-xl font-bold p-4 border-b border-gray-300 sticky top-0 bg-white z-10";
        rightPanel.appendChild(title);
        ...
        if (decorateInventory.length === 0) {
          ...
          noDecorMsg.textContent = "Không còn đồ trang trí nào để chọn.";
          ...
          decorateInventory.forEach((dec) => {
            ...
            btn.setAttribute("aria-label", `Thêm đồ trang trí: ${dec.name}`);
      ...
      function renderSelectPlate() {
        ...
        if (plateInventory.length === 0) {
          ...
          noPlateMsg.textContent = "Không còn đĩa nào để chọn.";
      ...
      function renderDecorations() {
        ...
        selectedDecorations.forEach((dec, index) => {
          ...
          decEl.setAttribute("aria-label", `Đồ trang trí: ${dec.name}`);
      ...
      function renderHistory() {
        ...
        if (history.length === 0) {
          ...
          emptyMsg.textContent = "Chưa có bánh đã trang trí nào trong lịch sử.";
        ...
        history.forEach((item) => {
          ...
          plateName.textContent = `Đĩa: ${item.plate.name}`;
          ...
          decText.textContent = `Trang trí: ${decNames}`;
      ...
      function loadGoogleUser() {
        try {
          const googleUserRaw = sessionStorage.getItem("googleuser");
          if (!googleUserRaw) {
            userAvatar.src = "https://placehold.co/32x32?text=U&font=roboto";
            userAvatar.alt = "Ảnh đại diện mặc định";
            userName.textContent = "Chưa đăng nhập";
            return;
          }
          const googleUser = JSON.parse(googleUserRaw);
          if (googleUser && googleUser.name && googleUser.picture) {
            userAvatar.src = googleUser.picture;
            userAvatar.alt = `Ảnh đại diện của ${googleUser.name}`;
